...
namespace lwrcl
{
  ...
  public:
    ...
    // Getters
    std::shared_ptr<AutosarDomainParticipant> get_participant() const;
    ...
    std::shared_ptr<Subscription<T>> create_subscription(
        const std::string &topic, const QoS &qos, CallbackT &&callback_function, Args &&...)
    {
      auto callback = detail::make_subscription_callback<T>(std::forward<CallbackT>(callback_function), 0);
      auto subscription = std::make_shared<Subscription<T>>(
          participant_.get(), resolve_topic_name(topic), qos, std::move(callback), callback_mutex_);
      subscription_list_.push_front(subscription);
      return subscription;
    }
  ...
  namespace executors
  {
    ...
    // Executor that manages and executes nodes, each in its own thread.
  ...
  public:
    ...
    Service(
        AutosarDomainParticipant *participant, const std::string &service_name,
        std::function<void(std::shared_ptr<typename T::Request>, std::shared_ptr<typename T::Response>)>
            callback_function,
