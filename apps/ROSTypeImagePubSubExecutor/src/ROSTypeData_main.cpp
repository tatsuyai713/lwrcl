...
int main(int argc, char **argv) {
    ...
    ROSTypeImagePubSubExecutor rcl_like_node1(0);
    ...
    if (rcl_like_node1.init(configPath1)) {
        executor.add_node(rcl_like_node1.get_node_pointer());
    } else {
        std::cerr << "Failed to initialize the ROSTypeImagePubSubExecutor." << std::endl;
        return 1;
    }
    ROSTypeImagePubSubExecutor rcl_like_node2(0);
    ...
    if (rcl_like_node2.init(configPath2)) {
        executor.add_node(rcl_like_node2.get_node_pointer());
    } else {
        std::cerr << "Failed to initialize the ROSTypeImagePubSubExecutor." << std::endl;
        return 1;
    }
