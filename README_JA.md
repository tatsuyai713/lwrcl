...
> サブスクリプションはイベント駆動型の `WaitSet` + `ReadCondition` を使用（ポーリングなし）。
> 生 CycloneDDS との残差はコールバックディスパッチ層のオーバーヘッドです。
...
レイテンシは C++ rclcpp（または lwrcl）パブリッシャー/サブスクライバーのピンポンペアで計測し、
メッセージ型は `std_msgs::msg::String` です。
**各ペアは同一の DDS バックエンドを使用**し、ミドルウェアラッパーのオーバーヘッドのみを比較しています。
...
| 指標 | lwrcl + CycloneDDS（コールバック） | ROS2 + CycloneDDS（rclcpp） | ROS2 + FastDDS（rclcpp） |
|-----|----------------------------------:|----------------------------:|------------------------:|
| p50 | ~148 µs | **~22 µs** | ~32 µs |
| p90 | ~275 µs | **~78 µs** | ~118 µs |
| p99 | ~322 µs | **~280 µs** | ~375 µs |
| 最小 | ~61 µs | **~14 µs** | ~20 µs |

> lwrcl はイベント駆動型の `WaitSet` + `ReadCondition` を使用（ポーリングなし）。ROS2+CycloneDDS との
> 差はチャネルベースのコールバックディスパッチ層のオーバーヘッドです。p99 では lwrcl と
> ROS2+CycloneDDS はほぼ同等（~322 µs vs ~280 µs）です。

### まとめ

| 優位性 | 勝者 |
|--------|------|
| ミドルウェアサイズ | **lwrcl** (~4 MB vs 6–15 MB) |
| メモリフットプリント（RSS） | **lwrcl** (~15 MB vs 21–38 MB) |
| コールバック API レイテンシ (p50) | ROS2 (~22 µs vs lwrcl ~148 µs) |
| コールバック API レイテンシ (p99) | ほぼ同等 (~280 µs vs ~322 µs) |
| WaitSet / 生 DDS レイテンシ | **lwrcl** (CycloneDDS 直接アクセス) |
| 移植性（非 Linux ターゲット） | **lwrcl** (QNX、組み込み、AUTOSAR) |
