...
以下の数値はすべて **x86\_64 Linux ホスト**（Docker コンテナ、Ubuntu 22.04）上で計測しました。
レイテンシテストは C++ プロセス内ピンポンノード（`std_msgs::msg::String`、1 ms タイマー、1000 サンプル、200 ウォームアップ）を使用しています。
...
### 3. エンドツーエンドレイテンシ（コールバック API、4 パターン比較）

C++ プロセス内ピンポンノード（`std_msgs::msg::String`、1 ms タイマー、1000 サンプル、200 ウォームアップ）で計測。
数値は**往復遅延 ÷ 2**（片道レイテンシ）で、各値は同一 Docker ホスト上での 5 回計測の中央値です。

| 指標   | lwrcl + CycloneDDS | lwrcl + FastDDS | ROS 2 + CycloneDDS | ROS 2 + FastDDS |
|-------|-------------------:|----------------:|------------------:|---------------:|
| p50   | ~75 µs             | ~71 µs          | ~57 µs            | ~79 µs         |
| p90   | ~157 µs            | ~179 µs         | ~141 µs           | ~183 µs        |
| p99   | ~449 µs            | ~482 µs         | ~456 µs           | ~625 µs        |
| 最小   | ~12 µs             | ~15 µs          | ~10 µs            | ~15 µs         |
...
>
> **p99 について**: Docker コンテナ環境では、p99 レイテンシは DDS やミドルウェアのオーバーヘッドより
> OS スケジューラーのジッターに支配されており、実行ごとに大きく変動します。
> 4 バターンすべてで同程度（~450–625 µs）となっており、p50・p90 がより安定した代表的な指標です。
...
レイテンシは C++ プロセス内ピンポンノード（`std_msgs::msg::String`、1000 サンプル、200 ウォームアップ）で計測し、
数値は**往復遅延 ÷ 2**（片道）・5 回計測の**中央値**です。

### ミドルウェア層のサイズ

| コンポーネント | lwrcl + CycloneDDS | lwrcl + FastDDS | ROS 2 + CycloneDDS | ROS 2 + FastDDS |
|-------------|-------------------:|----------------:|------------------:|---------------:|
| ラッパー / rclcpp 層 | **811 KB** | **1.1 MB** | 3.0 MB | 3.0 MB |
| DDS ランタイム | 3.0 MB | 12 MB | 3.0 MB | 12 MB |
| **合計** | **~3.8 MB** | **~13 MB** | **~6.0 MB** | **~15 MB** |

### メモリフットプリント（RSS、サブスクライバーノード起動時）
...
> ノードレベルのミューテックスで並行コールバックをシリアライズします。
>
> **p99 について**: Docker コンテナ環境では、p99 レイテンシは OS スケジューラーのジッターに支配されており、
> 実行ごとに大きく変動します。4 パターンすべてで同程度（~450–625 µs）となっており、
> p50・p90 がより安定した代表的な指標です。

### まとめ

| 指標 | lwrcl + CycloneDDS | lwrcl + FastDDS | ROS 2 + CycloneDDS | ROS 2 + FastDDS |
|-----|:------------------:|:---------------:|:-----------------:|:--------------:|
| ミドルウェアサイズ | **~3.8 MB** ✅ | ~13 MB | ~6.0 MB | ~15 MB |
| メモリ（RSS） | **~15 MB** ✅ | ~32 MB | ~21 MB | ~38 MB |
| レイテンシ p50 | ~75 µs | ~71 µs | **~57 µs** | ~79 µs |
| レイテンシ p90 | ~157 µs | ~179 µs | **~141 µs** | ~183 µs |
| レイテンシ p99 | ~449 µs | ~482 µs | ~456 µs | ~625 µs |
| 移植性 | **QNX / AUTOSAR** ✅ | **QNX** ✅ | Linux のみ | Linux のみ |
| ROS 2 不要 | **Yes** ✅ | **Yes** ✅ | No | No |
