...
### 3. エンドツーエンドレイテンシ（lwrcl コールバック API）

C++ ピンポンノードで lwrcl パブリッシャー/サブスクリプション コールバック API を使用して計測
（プロセス内、CycloneDDS バックエンド、`std_msgs::msg::String`、Release ビルド）。
...
> **最小レイテンシが ROS2 + CycloneDDS（~14 µs）と同等になりました。**
...
レイテンシは C++ ピンポンペア（パブリッシャー＋サブスクライバーを同一プロセスで起動）で計測し、
メッセージ型は `std_msgs::msg::String`、数値は**往復遅延 ÷ 2**（片道）です。
**両サイドは同一の DDS バックエンドを使用**し、ミドルウェアラッパーのオーバーヘッドのみを比較しています。

### ミドルウェア層のサイズ

| コンポーネント | lwrcl + CycloneDDS | ROS2 + CycloneDDS | ROS2 + FastDDS |
|-------------|-------------------:|------------------:|---------------:|
| ラッパー / rclcpp 層 | **811 KB** | 3.0 MB | 3.0 MB |
| DDS ランタイム | 3.0 MB | 3.0 MB | 12 MB |
| **合計** | **~3.8 MB** | **~6.0 MB** | **~15 MB** |
...
| 指標 | lwrcl + CycloneDDS | ROS2 + CycloneDDS | ROS2 + FastDDS |
|-----|-------------------:|------------------:|---------------:|
| p50 | ~68 µs | **~22 µs** | ~32 µs |
| p90 | ~107 µs | **~78 µs** | ~118 µs |
| p99 | **~161 µs** | ~280 µs | ~375 µs |
| 最小 | **~14 µs** | **~14 µs** | ~20 µs |
...
> ノードレベルのミューテックスで並行コールバックをシリアライズし、シングルスレッドのセマンティクスを提供します。
> **最小レイテンシが ROS2+CycloneDDS（~14 µs）と同等になりました。**
> **p99 では lwrcl（~161 µs）は ROS2+CycloneDDS（~280 µs）・ROS2+FastDDS（~375 µs）より大幅に高速**です。

### まとめ

| 指標 | lwrcl + CycloneDDS | ROS2 + CycloneDDS | ROS2 + FastDDS |
|-----|:------------------:|:-----------------:|:--------------:|
| ミドルウェアサイズ | **~3.8 MB** ✅ | ~6.0 MB | ~15 MB |
| メモリ（RSS） | **~15 MB** ✅ | ~21 MB | ~38 MB |
| レイテンシ p50 | ~68 µs | **~22 µs** ✅ | ~32 µs |
| レイテンシ p99 | **~161 µs** ✅ | ~280 µs | ~375 µs |
| 移植性 | **QNX / AUTOSAR** ✅ | Linux のみ | Linux のみ |
| ROS2 不要 | **Yes** ✅ | No | No |
