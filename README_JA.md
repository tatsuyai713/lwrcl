...
> サブスクリプションはイベント駆動型の `WaitSet` + `ReadCondition` を使用し、コールバックを
> WaitSet スレッドから直接呼び出します（エグゼキューター経由なし）。ノードレベルの
> ミューテックスで並行コールバックをシリアライズします。
...
| 最小 | ~30 µs | **~14 µs** | ~20 µs |

> lwrcl のコールバックは DDS WaitSet スレッドから直接呼び出します（エグゼキューター経由なし）。
> ノードレベルのミューテックスで並行コールバックをシリアライズし、シングルスレッドのセマンティクスを提供します。
> p50 での差はスレッドウェイクアップ 1 回分のオーバーヘッドです。
> **p99 では lwrcl（~220 µs）は ROS2+CycloneDDS（~280 µs）・ROS2+FastDDS（~375 µs）より高速**です。
...
| レイテンシ p50 | ~95 µs | **~22 µs** ✅ | ~32 µs |
| レイテンシ p99 | **~220 µs** ✅ | ~280 µs | ~375 µs |
